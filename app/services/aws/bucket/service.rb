  ...
  module Bucket
    class Service
      def ingest_status(identifier) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
        ...
        Rails.logger.error "Aws::Service.ingest_status(#{identifier}) #{e}"
        'standard_error'
      end
      ...
      def initialize(options = {})
        @base = options[:base]
        @base ||= begin
          filename = Rails.root.join('config', 'aws.yml')
        ...
        def connection
          @connection ||= Faraday.new(@base) do |conn|
            conn.headers = {
                ...
                "X-Pharos-API-User" => @yaml['AwsApiUser'],
                "X-Pharos-API-Key" => @yaml['AwsApiKey']
            }
            conn.request :json
